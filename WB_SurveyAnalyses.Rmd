...
model <- aov(logit.Llast.lpc.intensity ~ month.collected*depth, data = filter(wb.quad, depth != "mid"))
...
bargraph.CI(x.factor = month.collected,
            ...
            #group = depth,
            data = filter(wb.quad, depth != "mid"),
            ylim = c(0,3),
            main = "Field oldest leaf in sheath intensity",
            ylab = "Relative lesion percent cover", legend = T)

model <- aov(field.est.intensity ~ month.collected*depth, data = filter(wb.quad, depth != "mid"))
anova(model)
```
...
wb_temp$temp.C.10yr <- wb_temp$temp.C+2.3

#Full time series
ggplot(filter(wb_temp, transect == "A"), aes(x = datetimestamp)) + 
  geom_line(aes(y = temp.C), color = "black") +
  geom_line(aes(y = temp.C.10yr), color = "grey", linetype = "dashed") + 
