...
##binom model
model <- glmer(cbind(wd.present, wd.absent) ~ month.collected*depth + (1|month.collected/depth/quadrat.id), na.action = na.omit, data = wb.5shoot.binom.ds, family = binomial)
Anova(model)
...
wb.quad$WSWI <- NA #Wholoe shoot wasting index from Burdick et al. 1993
for (n in 1:72) {
...
##NMDS Categorical intensity of oldest leaf in sheath
```{r}
...
community <- subset(Llast.lpc.cat, select = c("prop.trace",
                                        "prop.low",
                                        "prop.mid"))
## --- nMDS Analysis/figures ---
set.seed(1)
...
# Plot each region's site scores in a different color and shape
points(quad.scores[1:3,], pch = 21, bg = mycols.month[1], col = mycols.month[1])
points(quad.scores[4:6,], pch = 22, bg = mycols.month[2], col = mycols.month[2])

month.names <- paste(levels(Llast.lpc.cat$season)) #, "month")
...
anova(model, test = "LR") #significant effect of depth, fewer infected at deep depth compared to shallow depth
